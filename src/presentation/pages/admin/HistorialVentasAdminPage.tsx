    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground uppercase">Historial de Ventas</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className="border-border/30 bg-muted/30 backdrop-blur-md">
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Eventos Registrados</p>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-muted/10 border border-border/30 rounded-2xl">
          ...
          <Button onClick={openCreate} className="mt-6 bg-primary/90 hover:bg-primary text-primary-foreground gap-2 text-xs font-bold uppercase"><Plus className="size-4" /> Registrar Evento</Button>
        </div>
      ) : (
        <Card className="border-border/30 bg-muted/10 backdrop-blur-md overflow-hidden">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {historiales.map((h) => (
                  <TableRow key={h.id_historial} className="hover:bg-muted/20 border-b border-border/20">
                    <TableCell className="font-mono font-bold text-muted-foreground">#{h.id_historial}</TableCell>
                    <TableCell className="font-mono text-foreground">#{h.id_venta}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-muted-foreground uppercase bg-muted px-2 py-0.5 rounded">{h.estado_anterior}</span>
                    ...
                    <TableCell className="text-muted-foreground text-xs">{formatDate(h.fecha_cambio)}</TableCell>
                    <TableCell className="text-muted-foreground text-xs max-w-[200px] truncate">{h.observacion || '—'}</TableCell>
                    <TableCell>
          ...
          <div className="relative w-full max-w-lg bg-card border border-border/40 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between px-6 py-5 border-b border-border/30 bg-muted/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-extrabold text-foreground tracking-tight">{editingId !== null ? 'Editar Evento' : 'Registrar Evento'}</h2>
                  <p className="text-xs text-muted-foreground mt-0.5">{editingId !== null ? `Registro #${editingId}` : 'Añadir cambio de estado manual'}</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Estado Anterior <span className="text-primary">*</span></label>
                  <input type="text" placeholder="Ej: procesando" value={form.estado_anterior} onChange={(e) => setForm((p) => ({ ...p, estado_anterior: e.target.value }))}
                    className={`w-full bg-background border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none transition-colors ${errs.estado_anterior ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                  />
                  {errs.estado_anterior && <p className="text-xs text-destructive">{errs.estado_anterior}</p>}
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Estado Nuevo <span className="text-primary">*</span></label>
                  <input type="text" placeholder="Ej: enviada" value={form.estado_nuevo} onChange={(e) => setForm((p) => ({ ...p, estado_nuevo: e.target.value }))}
                    className={`w-full bg-background border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none transition-colors ${errs.estado_nuevo ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                  />
