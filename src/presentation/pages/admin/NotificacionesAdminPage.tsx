...
export default function NotificacionesAdminPage() {
  ...
  const load = useCallback(() => { 
    fetchNotificaciones(); 
    fetchUsers(1000); 
  }, [fetchNotificaciones, fetchUsers]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground uppercase">Notificaciones</h1>
          <p className="text-muted-foreground text-sm">Gestiona las notificaciones y correos de los usuarios</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button onClick={openMasivo} className="bg-muted hover:bg-neutral-700 text-foreground font-bold uppercase tracking-wider text-xs gap-2 shrink-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className="border-border/30 bg-muted/30 backdrop-blur-md">
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Total Notificaciones</p>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-muted/10 border border-border/30 rounded-2xl">
          ...
          <Button onClick={openCreate} className="mt-6 bg-primary/90 hover:bg-primary text-primary-foreground gap-2 text-xs font-bold uppercase"><Plus className="size-4" /> Crear Notificación</Button>
        </div>
      ) : (
            ...
            <Table>
              ...
              <TableBody>
                {notificaciones.map((n) => (
                  <TableRow key={n.id_notificacion} className={`border-b border-border/20 transition-colors ${n.leido ? 'bg-background/30' : 'bg-primary/5 hover:bg-primary/10'}`}>
                    <TableCell className="font-mono font-bold text-muted-foreground">#{n.id_notificacion}</TableCell>
                    <TableCell className="font-mono">#{n.id_usuario}</TableCell>
                    <TableCell>
                      <div className="font-bold text-foreground">{n.titulo}</div>
                      <div className="text-xs text-muted-foreground line-clamp-1 mt-0.5">{n.mensaje}</div>
                    </TableCell>
                    <TableCell>
                      {n.leido 
                    ...
                    <TableCell className="text-muted-foreground text-xs">{formatDate(n.fecha_creacion)}</TableCell>
                    <TableCell>
                      <div className="flex items-center justify-center gap-1 flex-wrap">
                        ...
                        <Button variant="ghost" size="icon-sm" onClick={() => openEdit(n.id_notificacion)} className="text-muted-foreground hover:bg-neutral-500/10"><Pencil className="size-4" /></Button>
          ...
          <div className="relative w-full max-w-lg bg-card border border-border/40 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between px-6 py-5 border-b border-border/30 bg-muted/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-extrabold text-foreground tracking-tight">{editingId !== null ? 'Editar Notificación' : 'Crear Notificación'}</h2>
                  <p className="text-xs text-muted-foreground mt-0.5">{editingId !== null ? `Registro #${editingId}` : 'Enviar a un usuario específico'}</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Usuario <span className="text-primary">*</span></label>
                <select
                  value={form.id_usuario}
                  onChange={(e) => setForm((p) => ({ ...p, id_usuario: e.target.value }))}
                  className={`w-full bg-background border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none transition-colors ${errs.id_usuario ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                >
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Título <span className="text-primary">*</span></label>
                <input type="text" placeholder="Asunto de la notificación" value={form.titulo} onChange={(e) => setForm((p) => ({ ...p, titulo: e.target.value }))}
                  className={`w-full bg-background border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none transition-colors ${errs.titulo ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                />
                {errs.titulo && <p className="text-xs text-destructive">{errs.titulo}</p>}
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Mensaje <span className="text-primary">*</span></label>
                <textarea rows={4} placeholder="Escribe el mensaje..." value={form.mensaje} onChange={(e) => setForm((p) => ({ ...p, mensaje: e.target.value }))}
                  className={`w-full bg-background border rounded-lg px-4 py-2.5 text-sm text-foreground focus:outline-none transition-colors resize-none ${errs.mensaje ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                />
      ...
      {showMasivo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-card border border-blue-500/40 rounded-2xl shadow-2xl shadow-blue-500/10 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between px-6 py-5 border-b border-border/30 bg-blue-500/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-extrabold text-foreground tracking-tight">Enviar Notificación Masiva</h2>
                  <p className="text-xs text-blue-200/60 mt-0.5">Se enviará un correo a todos los usuarios</p>
                </div>
              </div>
              <Button variant="ghost" size="icon-sm" onClick={closeMasivo} className="text-muted-foreground hover:text-foreground"><X className="size-5" /></Button>
            </div>
            ...
            <form onSubmit={handleMasivoSubmit} className="p-6 space-y-4">
              <div className="space-y-1.5">
                ...
                <input type="text" placeholder="Promoción, aviso importante..." value={mForm.titulo} onChange={(e) => setMForm((p) => ({ ...p, titulo: e.target.value }))}
                  className={`w-full bg-background border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none transition-colors ${mErrs.titulo ? 'border-destructive' : 'border-blue-500/30 focus:border-blue-500'}`}
                />
              ...
              <div className="space-y-1.5">
                ...
                <textarea rows={5} placeholder="Escribe el mensaje que se enviará por correo a todos los usuarios..." value={mForm.mensaje} onChange={(e) => setMForm((p) => ({ ...p, mensaje: e.target.value }))}
                  className={`w-full bg-background border rounded-lg px-4 py-2.5 text-sm text-foreground focus:outline-none transition-colors resize-none ${mErrs.mensaje ? 'border-destructive' : 'border-blue-500/30 focus:border-blue-500'}`}
                />
              ...
              <div className="flex gap-3 pt-4 border-t border-border/20 mt-4">
                <Button type="button" variant="outline" onClick={closeMasivo} className="flex-1 border-border/40 text-muted-foreground hover:text-foreground text-xs font-bold uppercase tracking-wider">Cancelar</Button>
                <Button type="submit" disabled={isSaving} className="flex-1 bg-blue-600 hover:bg-blue-500 text-foreground font-bold text-xs uppercase tracking-wider gap-2">
