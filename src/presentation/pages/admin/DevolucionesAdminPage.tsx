    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-muted/10 border border-border/30 rounded-2xl">
          ...
          <Button onClick={openCreate} className="mt-6 bg-primary/90 hover:bg-primary text-primary-foreground gap-2 text-xs font-bold uppercase"><Plus className="size-4" /> Registrar Devolución</Button>
        </div>
      ) : (
              ...
              <TableBody>
                {devoluciones.map((d) => (
                  <TableRow key={d.id_devolucion} className="hover:bg-muted/20 border-b border-border/20">
                    ...
                    <TableCell className="text-muted-foreground text-xs max-w-[200px] truncate" title={d.motivo}>{d.motivo}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center justify-center gap-1 flex-wrap">
                        ...
                        <Button variant="ghost" size="icon-sm" onClick={() => openEdit(d.id_devolucion)} className="text-muted-foreground hover:bg-neutral-500/10"><Pencil className="size-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
          ...
          <div className="relative w-full max-w-lg bg-card border border-border/40 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between px-6 py-5 border-b border-border/30 bg-muted/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-extrabold text-foreground tracking-tight">{editingId !== null ? 'Editar Devolución' : 'Registrar Devolución'}</h2>
                  <p className="text-xs text-muted-foreground mt-0.5">{editingId !== null ? `Registro #${editingId}` : 'Completa los datos de la solicitud'}</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Monto a Devolver <span className="text-primary">*</span></label>
                  <div className="relative">
                    ...
                    <input type="number" step="0.01" min="0.01" placeholder="0.00" value={form.monto_devolucion} onChange={(e) => setForm((p) => ({ ...p, monto_devolucion: e.target.value }))}
                      className={`w-full bg-background border rounded-lg pl-6 pr-2 py-2.5 text-sm text-foreground focus:outline-none transition-colors ${errs.monto_devolucion ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                    />
                  </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Motivo <span className="text-primary">*</span></label>
                <textarea rows={3} placeholder="Describe detalladamente el motivo de la devolución..." value={form.motivo} onChange={(e) => setForm((p) => ({ ...p, motivo: e.target.value }))}
                  className={`w-full bg-background border rounded-lg px-4 py-2.5 text-sm text-foreground focus:outline-none transition-colors resize-none ${errs.motivo ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                />
