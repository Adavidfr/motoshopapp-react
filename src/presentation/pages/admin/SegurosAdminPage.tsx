    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-extrabold tracking-tight text-white uppercase">Módulo de Seguros</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-border/30 bg-neutral-900/30 backdrop-blur-md">
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Total Seguros</p>
        ...
        <Card className="border-border/30 bg-neutral-900/30 backdrop-blur-md">
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Pólizas Activas</p>
        ...
        <Card className="border-border/30 bg-neutral-900/30 backdrop-blur-md">
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Costo Anual Total</p>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-neutral-900/10 border border-border/30 rounded-2xl">
          ...
          <h3 className="text-lg font-bold text-white">No se encontraron seguros</h3>
          <Button onClick={openCreate} className="mt-6 bg-primary/90 hover:bg-primary text-white gap-2 text-xs font-bold uppercase"><Plus className="size-4" /> Nuevo Seguro</Button>
        </div>
      ) : (
        <Card className="border-border/30 bg-neutral-900/10 backdrop-blur-md overflow-hidden">
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {seguros.map((s) => (
                  <TableRow key={s.id_seguro} className="hover:bg-neutral-900/20 border-b border-border/20">
                    <TableCell className="font-mono font-bold text-neutral-400">#{s.id_seguro}</TableCell>
                    <TableCell className="font-mono">#{s.id_venta}</TableCell>
                    <TableCell className="font-bold text-white">{s.aseguradora}</TableCell>
                    <TableCell className="text-neutral-300 font-mono text-xs">{s.numero_poliza}</TableCell>
                    <TableCell className="text-neutral-300 text-xs">{s.tipo_cobertura}</TableCell>
                    <TableCell className="text-neutral-400 text-xs">{s.fecha_inicio}</TableCell>
                    <TableCell className="text-neutral-400 text-xs">{s.fecha_fin}</TableCell>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-[#0e0e10] border border-border/40 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between px-6 py-5 border-b border-border/30 bg-neutral-900/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-extrabold text-white tracking-tight">{editingId !== null ? 'Editar Seguro' : 'Registrar Seguro'}</h2>
                  <p className="text-xs text-neutral-400 mt-0.5">{editingId !== null ? `Seguro #${editingId}` : 'Completa los datos de la póliza'}</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-neutral-300 uppercase tracking-wider">Aseguradora <span className="text-primary">*</span></label>
                  <input type="text" placeholder="Nombre de la aseguradora" value={form.aseguradora} onChange={(e) => setForm((p) => ({ ...p, aseguradora: e.target.value }))}
                    className={`w-full bg-neutral-950 border rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none transition-colors ${errs.aseguradora ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-neutral-300 uppercase tracking-wider">N° Póliza <span className="text-primary">*</span></label>
                  <input type="text" placeholder="POL-2026-001" value={form.numero_poliza} onChange={(e) => setForm((p) => ({ ...p, numero_poliza: e.target.value }))}
                    className={`w-full bg-neutral-950 border rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none transition-colors ${errs.numero_poliza ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                  />
                  {errs.numero_poliza && <p className="text-xs text-destructive">{errs.numero_poliza}</p>}
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-neutral-300 uppercase tracking-wider">Tipo de Cobertura <span className="text-primary">*</span></label>
                  <input type="text" placeholder="Todo Riesgo / Básica / Terceros" value={form.tipo_cobertura} onChange={(e) => setForm((p) => ({ ...p, tipo_cobertura: e.target.value }))}
                    className={`w-full bg-neutral-950 border rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none transition-colors ${errs.tipo_cobertura ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                  />
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-neutral-300 uppercase tracking-wider">Costo Anual <span className="text-primary">*</span></label>
                  <div className="relative">
                    ...
                    <input type="number" step="0.01" min="0.01" placeholder="0.00" value={form.costo_anual} onChange={(e) => setForm((p) => ({ ...p, costo_anual: e.target.value }))}
                      className={`w-full bg-neutral-950 border rounded-lg pl-6 pr-2 py-2.5 text-sm text-white focus:outline-none transition-colors ${errs.costo_anual ? 'border-destructive' : 'border-border/30 focus:border-primary'}`}
                    />
                  </div>
