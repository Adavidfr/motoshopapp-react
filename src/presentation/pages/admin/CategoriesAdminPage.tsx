// src/presentation/pages/admin/CategoriesAdminPage.tsx
import { useEffect, useState } from 'react';
import { useCategoryStore } from '../../store/category.store';
import { Plus, ToggleLeft, ToggleRight, Loader2, Edit } from 'lucide-react';

export default function CategoriesAdminPage() {
  const { categories, isLoading, error, fetchCategories, createCategory, updateCategory } = useCategoryStore();
  ...
  const handleOpenEdit = (category: any) => {
    ...
    setFormDesc(category.descripcion || '');
    setFormStatus(category.estado);
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId !== null) {
        await updateCategory(editingId, { nombre: formName, descripcion: formDesc, estado: formStatus });
      } else {
        await createCategory({ nombre: formName, descripcion: formDesc, estado: formStatus });
      }
      setModalOpen(false);
    } catch (err) {
  ...
  const handleToggleStatus = async (category: any) => {
    try {
      await updateCategory(category.idCategoria, { estado: !category.estado });
    } catch (err) {
      // Manejado
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 py-6 px-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-foreground uppercase tracking-tight">Administrar Categorías</h1>
          <p className="text-muted-foreground text-sm">Gestiona los tipos de motocicletas del catálogo</p>
        </div>
        <button
          ...
          <table className="w-full border-collapse text-left">
            ...
            <tbody className="divide-y divide-neutral-950 text-sm">
              {categories.map((category) => (
                <tr key={category.idCategoria} className="hover:bg-muted/20 transition-colors">
                  <td className="py-4 px-6 font-bold text-neutral-500">#{category.idCategoria}</td>
                  <td className="py-4 px-6 font-bold text-foreground">{category.nombre}</td>
                  <td className="py-4 px-6 text-muted-foreground max-w-xs truncate">{category.descripcion || 'Sin descripción'}</td>
                  <td className="py-4 px-6 text-center">
                    <button onClick={() => handleToggleStatus(category)} className="focus:outline-none">
                      {category.estado ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  required
                  placeholder="Ej: Deportivas, Scooters, Enduro"
                  value={formName}
              ...
              <div className="space-y-1">
                ...
                <textarea
                  placeholder="Detalles sobre el tipo de motocicleta..."
                  value={formDesc}
