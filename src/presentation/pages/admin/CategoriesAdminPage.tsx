  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 py-6 px-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-white uppercase tracking-tight">Administrar Categorías</h1>
          <p className="text-neutral-400 text-sm">Gestiona los tipos de motocicletas del catálogo</p>
        </div>
        <button
          ...
          <table className="w-full border-collapse text-left">
            ...
            <tbody className="divide-y divide-neutral-950 text-sm">
              {categories.map((category) => (
                <tr key={category.idCategoria} className="hover:bg-neutral-900/20 transition-colors">
                  <td className="py-4 px-6 font-bold text-neutral-500">#{category.idCategoria}</td>
                  <td className="py-4 px-6 font-bold text-white">{category.nombre}</td>
                  <td className="py-4 px-6 text-neutral-400 max-w-xs truncate">{category.descripcion || 'Sin descripción'}</td>
